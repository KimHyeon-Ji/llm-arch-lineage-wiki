...
마지막 줄이 미해결 영역이다. `02-linear-attention`의 고정 상태는 시퀀스 전체에
의존하므로, prefix 지점의 상태를 스냅샷해두는 방식이 되어야 한다.
상태가 고정 크기라 저장 자체는 오히려 쉬울 수 있지만,
**실제 프레임워크 지원 상황은 확인이 필요하다.**
...
**미검증 항목**
- 하이브리드(linear + full) 모델에서의 prefix caching 지원 상황 — 프레임워크별 확인 필요
- MLA의 TP 분할 방식 세부 — 구현 코드 대조 필요
- `9.7` 표의 구분선(어느 배치·컨텍스트에서 전환되는지) — 정성적 서술이며 구체 수치는 하드웨어와 모델에 따라 다름
