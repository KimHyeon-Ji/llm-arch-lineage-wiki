# 09. Serving — 실제로 돌릴 때 벌어지는 일

앞의 여덟 파일은 **모델이 어떻게 생겼는가**를 다뤘다.
이 파일은 그 모델을 GPU에 올려 실제 사용자를 받을 때 벌어지는 일을 다룬다.

여기서 앞의 결정들이 전부 청구서로 돌아온다.

- MLA를 쓰면 KV 페이지 구조가 달라진다 (`9.2`)
- linear 하이브리드를 쓰면 층마다 KV 할당이 달라진다 (`9.2`)
- MoE를 쓰면 all-to-all이 병목이 된다 (`9.6`)
- GQA의 `n_kv`가 TP 분할 가능한 GPU 수를 제한한다 (`9.6`)
- 층수가 많으면 지연이 늘고, 이건 GPU를 붙여도 안 줄어든다 (`9.6`)
...
| | 절 | 한 줄 |
|---|---|---|
| [9.1](#91-두-단계는-정말-다른-작업이다) | **prefill vs decode** | 정말 다른 작업이다 |
| [9.2](#92-pagedattention--kv를-페이지로) | **PagedAttention** | KV를 페이지로 |
| [9.3](#93-prefix-caching--아예-계산하지-않기) | **prefix caching** | 아예 계산하지 않기 ★ |
| [9.4](#94-continuous-batching과-chunked-prefill) | **배칭과 chunked prefill** | 언제 무엇을 실행할까 |
| [9.5](#95-pd-disaggregation--아예-분리하기) | **PD disaggregation** | 아예 분리하기 |
| [9.6](#96-병렬화--무엇을-쪼개고-무엇을-주고받나) | **병렬화** | 무엇을 쪼개고 주고받나 ★ |
| [9.7](#97-종합--무엇이-먼저-터지는가) | **종합** | 무엇이 먼저 터지는가 ★ |

---

## 9.1 두 단계는 정말 다른 작업이다
...
`9.4`와 `9.5`가 이 문제에 대한 두 가지 답이다.

---

## 9.2 PagedAttention — KV를 페이지로
...
```
 ...
 토큰 32~47  ──────────►  블록 12   ──────►  [블록 12]
                                             (연속일 필요가 없다)
```
> **그림 9.2** — 논리적으로 연속인 KV가 물리적으로는 흩어져 있어도 된다.
...
| **페이지 공유** | 같은 내용의 페이지를 여러 요청이 공유할 수 있다 → `9.3` |
...
`9.2`의 페이지 공유가 그대로 쓰인다. 같은 prefix를 가진 요청들이 **같은 물리 페이지를
가리키게** 하면 된다.
...
```
> **그림 9.3** — radix tree로 관리되는 prefix 캐시
...
### ✅ 하이브리드 모델의 prefix caching — 확인된 상황
...
| 접근 | 무엇 |
|---|---|
| **Marconi** (arXiv:2411.19379) | 하이브리드 LLM용 prefix cache. **적절한 지점에서 linear attention 상태를 체크포인트**한다 |
| **SGLang MambaRadixCache** | radix tree를 하이브리드용으로 확장. **메모리 풀을 둘로 분리** — Mamba 풀은 **요청 단위**, KV 풀은 **토큰 단위** 할당 |

마지막 줄이 핵심이다. **할당 단위가 다르다.**
KV는 토큰마다 쪼갤 수 있지만 상태는 요청 하나가 통째로 갖는다.
그래서 `9.2`의 PagedAttention 모델을 그대로 못 쓰고 별도 풀이 필요하다.

> 📎 **vLLM과 SGLang 모두 처음에는 하이브리드/SSM prefix caching을 지원하지 않았다.**
> 나중에 추가된 기능이고, 지원 범위는 계속 바뀐다. **사용 전 현재 상태를 확인해야 한다.**
>
> 이건 아키텍처 변화가 서빙 시스템의 재작성을 요구한 가장 구체적인 사례다.
> `9.2` 마지막에서 예고한 문제가 실제로 벌어진 것이다.

---

## 9.4 continuous batching과 chunked prefill
...
`9.1`에서 본 간섭 문제에 대한 답이다. **긴 prefill을 조각내서 decode 사이에 끼워넣는다.**
...
**GEMV가 GEMM에 가까워진다.** `07-decoding`의 speculative decoding이 얻는 이득과
비슷한 성격이다.

---

## 9.5 PD disaggregation — 아예 분리하기
...
`08-numerics`의 KV 양자화가 여기서 다시 값을 한다.
...
| MoE는 **EP가 사실상 필수**이고 all-to-all이 따라온다 | `04` 4.6 |
| **깊이는 PP로 쪼개도 지연이 안 줄어든다** | `06` 6.1 |
...
`04-moe` `4.6`에서 본 것처럼 **노드를 넘는 통신이 노드 안보다 훨씬 느리므로**,
통신이 잦은 TP는 노드 안에, EP는 노드 간에 두는 식으로 배치한다.

📎 실측 범위로 보면 이 배치가 왜 정석인지 분명하다.
all-to-all이 차지하는 시간이 **노드 안이면 ~20%, 노드를 넘으면 40~60%** 로 보고된다.
EP 정도에 따라서도 **EP=2에서 30%, EP=6에서 77%** 까지 올라간다.
자세한 수치와 인용 주의사항은 `04-moe` `4.6`을 참조.
...
| 조건 | 먼저 터지는 것 | 대응 (파일) |
|---|---|---|
| 짧은 컨텍스트, **작은 배치** | 가중치 읽기 (대역폭) | 배치 키우기 · 양자화(`08`) · speculative(`07`) |
| 짧은 컨텍스트, **큰 배치** | 연산 (compute) | MoE(`04`) · 저정밀 연산(`08`) |
| 긴 컨텍스트, 작은 배치 | **KV 읽기** | KV 압축·희소(`01`) · linear(`02`) |
| **긴 컨텍스트, 큰 배치** | **KV 용량 — 아예 못 돌린다** | KV 압축(`01`) · KV 양자화(`08`) · PagedAttention(`9.2`) |
| MoE + 여러 노드 | **all-to-all 통신** | EP 배치 최적화 · 통신 중첩(`04` 4.6) |
| 층이 깊은 모델, 저지연 요구 | **직렬 지연** | 층수 줄이기(`06`) · speculative(`07`) |
| 멀티턴 · 에이전트 | prefill 반복 | **prefix caching(`9.3`)** |
| prefill과 decode 혼재 | 상호 간섭 | chunked prefill(`9.4`) · PD 분리(`9.5`) |
...
- 배치가 작으면 speculative decoding이 크게 이득이지만, 배치가 크면 이득이 줄어든다 (`07` 7.4)
- fine-grained MoE는 큰 배치에서 유리하고 작은 배치에서 불리하다 (`04` 4.4)
- 깊은 모델은 품질에 유리하고 지연에 불리하다 (`06` 6.1)
...
**T3 — 참고**
- 서빙 최적화 실무 가이드, 프레임워크 팀 블로그 — PD 분리의 운영 경험

**해소된 항목** ✅
- 하이브리드 모델의 prefix caching — Marconi(arXiv:2411.19379), SGLang MambaRadixCache,
  메모리 풀 분리(Mamba는 요청 단위 / KV는 토큰 단위)
- all-to-all 시간 비중 실측 범위 — `04-moe` 4.6

**미검증 항목**
- MLA의 TP 분할 방식 세부 — 구현 코드 대조 필요
- `9.7` 표의 구분선(어느 배치·컨텍스트에서 전환되는지) — 정성적 서술이며 구체 수치는
  하드웨어와 모델에 따라 다름
- **희소 attention의 gather 실효 대역폭** — 정성적 근거뿐이고 측정 자료를 찾지 못했다.
  all-to-all과 달리 이쪽은 여전히 추론이다
- 프레임워크별 하이브리드 prefix caching 지원 **현재 상태** — 계속 바뀐다
