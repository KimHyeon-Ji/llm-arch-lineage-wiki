# 03. Test-Time Memory — Titans · ATLAS · HOPE · Sleep
...
그리고 이 축은 여기서 두 번 더 밀고 나간다 — **갱신 규칙 자체를 학습 대상으로
만들고**(HOPE), 나아가 **갱신의 시점까지 옮긴다**(Sleep — 입력이 없는 시간에
기억을 정리한다).
...
> 맨 아래 Sleep만 **갱신의 시점**을 건드린다 — 앞의 셋은 전부 "무엇을 어떻게
> 갱신할까"였고, Sleep은 "언제 갱신할까"를 묻는다.
...
| [3.5](#35-sleep--깨어-있을-때와-잘-때를-나눈다) | **Sleep** | 잘 때 기억을 정리한다 |
| [3.6](#36-시스템-관점--추론-중에-학습이-일어난다) | **시스템 관점** | read-mostly에서 multi-rate read+write로 ★ |
...
| **Sleep** | 온라인 갱신만으로는 지식이 휘발되고 용량도 늘지 않음 | wake/sleep 분리 | 입력이 없는 시간대에 빠른 모듈 → 느린 파라미터로 증류 | CMS 위에 **오프라인 갱신 층**을 얹는다 |
...
| **대표 모델** | 연구 단계 (Google Research) |
| **다음으로** | 이 갱신들은 전부 **입력을 처리하는 중에** 일어난다. 입력이 없는 시간에 하면 → **Sleep** |

---

## 3.5 Sleep — 깨어 있을 때와 잘 때를 나눈다

*Language Models Need Sleep (2026) — Titans·ATLAS·HOPE와 같은 저자 그룹*

### 왜 나왔나

`3.4`의 CMS는 갱신 주기의 스펙트럼을 만들었다. 빠른 블록은 자주, 느린 블록은
드물게 갱신된다. 그런데 **아무리 느린 블록이라도 결국 입력을 처리하는 그 순간에
갱신된다.** 사용자가 답을 기다리는 동안, 토큰이 흐르는 그 경로 위에서.

그래서 두 가지가 남는다.

**① 얻은 지식이 휘발된다.** in-context로 배운 것은 문맥이 끝나면 사라진다.
`3.2`의 neural memory도 요청별 상태이지, 다음 대화로 넘어가지 않는다.

**② 온라인 갱신만으로는 용량이 늘지 않는다.** 같은 파라미터를 계속 덮어쓰는
것이라, 새 지식이 옛 지식을 밀어낸다 — catastrophic forgetting이다.

사람은 이 문제를 다르게 푼다. 깨어 있는 동안 겪은 것을 **자는 동안** 장기 기억으로
옮긴다. 낮에는 받아들이고, 밤에는 정리한다.

> **갱신을 꼭 입력을 처리하는 그 순간에 해야 하나?**

### 아이디어와 구조

📌 [T1] Sleep은 모델의 생애를 두 국면으로 나눈다.

```
 wake (깨어 있을 때)
   외부 입력을 받아 빠른 주기 모듈로 처리한다
   → 빠르게 배우지만 fragile하고 문맥에 묶인 지식

 sleep (잘 때)
   외부 입력이 (거의) 없는 동안, 이미 가진 지식만 가지고 두 가지를 한다
   ① consolidation — 빠른 모듈의 지식을 느린 모듈로 옮긴다
   ② dreaming      — 스스로 만든 데이터로 자신을 더 다듬는다
```
> **그림 3.5** — `3.4`의 CMS가 "주기가 다른 모듈들"이었다면, Sleep은 거기에
> **"입력을 받지 않는 시간대"** 라는 축을 하나 더 붙인다.

📌 [T1] 이 틀은 CMS 위에 그대로 올라간다. CMS에서 블록 `ℓ`은 `C^(ℓ) = max_C / f_ℓ`
스텝마다 갱신되는데, HOPE는 그 갱신을 **온라인 consolidation**(학습 중 직접 전달)으로
처리했다. Sleep은 여기에 **오프라인 consolidation**을 더한다 — 주기적으로 멈춰서
증류하는 단계다.

**consolidation — 덮어쓰지 않고 자리를 늘린다**

📌 [T1] 증류 전에 먼저, 받는 쪽(느린) MLP 블록에 **새 low-rank expert `{A, B}`를
추가**한다. 기존 파라미터를 고쳐 쓰는 대신 **차원을 늘리는 것**이고, 이것이 옛 지식을
지키는 핵심 장치다.

📌 [T1] 그 다음 세 단계를 밟는다 (논문은 이를 *knowledge seeding*이라 부른다).

```
 ① compute      보내는 쪽 블록이 자기 갱신량을 계산한다 (아직 적용하지 않는다)

 ② consolidate  갱신 전 모델        = teacher
                갱신된 sender + 새로 열린 target 파라미터 = student
                → student가 teacher를 따라가도록 증류
                  (on-policy distillation + RL 기반 imitation,
                   보상은 의미 유사도와 Levenshtein 거리를 섞은 것)

 ③ update       sender의 갱신을 실제로 적용하고,
                sender의 low-rank expert는 **리셋**한다 (synaptic pruning)
                target에 새로 학습된 expert는 **freeze**한다
```

방향이 거꾸로라는 점을 짚어야 한다. 보통 증류는 **큰 모델 → 작은 모델**이다.
여기는 **작은 자기가 큰 자기를 가르친다** — 논문의 표현으로 *upward distillation*이다.
목적이 압축이 아니라 **용량 확장**이기 때문이다. 방금 배운 것을 잃지 않은 채,
그것을 담을 그릇을 키우는 일이다.

> 💡 `3.4`에서 CMS가 "attention과 FFN 사이에 눈금을 그었다"면, Sleep은
> **그 눈금을 따라 지식을 실제로 이사시키는 절차**를 정의한 것에 가깝다.
> 스펙트럼만 있고 이동 규칙이 없으면, 느린 쪽 칸은 결국 학습 때 채워진 채로 남는다.

**dreaming — 스스로 만든 데이터로 복습한다**

📌 [T1] 두 번째 단계는 자기 생성 데이터로 자신을 개선하는 것이다.

```
 ① 생성   과제 문맥 C를 조건으로 m개의 시퀀스를 샘플한다
          이때 MoE 라우터가 **일부러 무작위 expert를 하나씩 더 고르게** 해서
          관련 없는 지식이 섞여 들어오게 한다 ← "꿈"에 해당하는 장치

 ② 선택   각 꿈에 대해 SFT 손실의 gradient 크기로 점수를 매긴다
          (gradient가 크다 = 아직 배울 것이 많은 데이터)
          Top-k + 다양성을 위한 무작위 b개를 고른다

 ③ 보상   꿈마다 **격리된 모델 사본**을 만들어 LoRA로 학습시켜 보고,
          downstream 성능이 실제로 좋아졌는지를 이진 보상으로 삼아
          생성 과정을 강화학습(ReST^EM)으로 최적화한다
```

> 💡 **자기 출력으로 자기를 학습시키면 보통 무너진다**(model collapse). 여기서
> 그것을 피하는 장치가 **생성과 본체 갱신을 분리**한 것이다 — 꿈은 격리된 사본에서
> 먼저 시험되고, 본체는 **효과가 확인된 것만** 받는다. `08-decoding` `8.1`의
> "draft는 마음대로 추측하되 target이 검증한다"와 구조가 닮았다. 값싼 생성과
> 신뢰할 수 있는 채점을 분리하는 패턴이다.

**잊지 않기 위해 쓰는 세 가지**

📌 [T1] 별도의 경험 replay 버퍼를 두는 대신, 구조로 푼다.

| 장치 | 효과 |
|---|---|
| 옛 파라미터는 freeze, **새 expert만 학습** | 이미 있는 지식을 물리적으로 건드리지 않는다 |
| **lossy distillation** | 원본을 외우는 게 아니라 추상화해서 담는다 |
| 여러 주기 층위로 **나눠 저장** | 한 곳이 포화돼도 다른 층위가 받는다 |

### 추론에서 달라진 것

| | HOPE (`3.4`) | + Sleep |
|---|---|---|
| 갱신 시점 | 입력을 처리하는 중 (온라인) | 온라인 + **입력이 없는 시간대 (오프라인)** |
| 파라미터 수 | 고정 | **잠들 때마다 low-rank expert가 늘어난다** |
| 지식의 수명 | 문맥·요청 단위 | **파라미터에 남는다** — 다음 세션으로 넘어간다 |
| 사용자 지연 | 갱신이 지연에 포함됨 | sleep은 지연 경로 밖 (⚠️ 대신 GPU 시간은 든다) |
| 새로 필요한 것 | — | **언제 재울지 정하는 스케줄러**, 꿈을 시험할 격리 인스턴스 |

### 하루가 지나가는 길

이 절만은 "토큰 하나"가 아니라 **한 주기**를 따라가는 게 맞다.

```
 [wake]   요청을 받아 서비스한다. 빠른 주기 모듈이 갱신되고,
          배운 것은 아직 문맥과 빠른 파라미터에만 있다
              │
              │  📌 [T1] sleep은 연속이 아니라 주기적으로 들어간다
              │  (CMS 주기의 배수 지점 {C^(1)·b, ..., C^(k)·b})
              ▼
 [sleep ①] 느린 블록에 새 low-rank expert를 붙여 자리를 만든다
 [sleep ②] 갱신 전 모델을 teacher로 삼아 새 자리로 증류한다
 [sleep ③] 갱신을 적용하고, 보낸 쪽은 리셋, 받은 쪽은 freeze
              │
              ▼
 [dream]  꿈을 만들고 → gradient 크기로 고르고 → 사본에서 시험하고
          → 효과가 확인된 것만 본체에 반영한다
              │
              ▼
 [wake]   더 두꺼워진 느린 블록을 가지고 다시 서비스로 돌아간다
```
...
| 과제 | 기준 | Sleep |
|---|---|---|
| 수학 추론 (Qwen3-8B) — AIME-24 / AIME-25 / HMMT-25 | 73.8 / 68.1 / 42.4 | **79.2 / 69.0 / 46.1** |
| 지식 주입 (SQuAD, 단일 passage) | SEAL 46.7% | **48.9%** |
| 지식 주입 (200 passage 연속) | SEAL 43.2% | **46.2%** |
| few-shot 추론 (ARC) | SEAL 72.5% · TTT 10% · ICL 0% | **80%** |
| 연속 번역 (CTNL, 두 언어를 순차로 학습) | ICL은 두 번째 언어에서 급락 | HOPE-3가 ~0.50 유지 |

📌 [T1] 장문 쪽에서는 consolidation 층위를 늘릴수록 성능이 단조적으로 좋아지고,
BABILong에서 1000만 토큰까지 확장해도 거의 만점을 보고한다.

⚠️ **비용 수치가 없다.** 논문은 sleep이 "입력이 없는 시간에 돈다"고만 하고,
**한 번 재우는 데 얼마나 걸리는지, FLOPs가 얼마인지, 메모리가 얼마나 더 드는지를
보고하지 않는다.** 운영 관점에서는 그게 바로 비용인데 그 칸이 비어 있다.

### 💡 시스템에서 새로 생기는 질문 (이 위키의 추정)

`3.6`의 분석을 Sleep에 적용하면, 이 축이 지금까지와 **성격이 다른** 문제를 하나
만든다는 게 보인다.

**모델이 더 이상 하나가 아니다.**

📌 지금까지(`3.2`~`3.4`)는 요청별로 달라지는 것이 **상태**였다. 요청이 끝나면
버리면 된다. Sleep에서 달라지는 것은 **파라미터**이고, 그것은 남는다. 사용자마다,
테넌트마다 각자의 밤을 보내고 나면 **가중치가 갈라진다.**

💡 (추정) `10-serving`의 서빙 스택은 **"가중치 하나를 모든 요청이 공유한다"** 를
전제로 만들어져 있다. LoRA 멀티테넌시가 이미 비슷한 문제를 다루지만, 거기서는
어댑터가 **사람이 만들어 배포하는 산출물**이다. 여기서는 갈라짐이 **자동으로,
계속** 일어난다.

| 새 질문 | 왜 생기나 |
|---|---|
| **언제 재우나** | 트래픽이 빈 시간대? 요청 수 기준? 테넌트별로 따로? — 재우는 동안의 GPU는 매출을 내지 않는다 |
| **누구의 밤인가** | 전역 모델 하나를 재울지, 테넌트마다 재울지에 따라 저장·라우팅 구조가 달라진다 |
| **잘못 배운 밤은 어떻게 되돌리나** | 파라미터가 바뀌었으니 롤백하려면 체크포인트나 델타 로그가 필요하다 |
| **검증은 누가 하나** | dreaming은 자기 평가로 자기를 바꾼다 — 평가 지표가 틀리면 그대로 굳는다 |

> 💡 `3.6`에서 볼 "미래 accelerator" 논의에 항목이 하나 더 붙는 셈이다 —
> **오프라인 consolidation을 위한 시간·자원을 어떻게 예약할 것인가.**
> 지금까지는 "추론 중에 학습이 들어온다"가 문제였다면, 여기서는
> **"학습을 위한 시간대를 서빙 계획에 넣어야 한다"** 가 문제가 된다.

### 같은 이름, 다른 갈래

📌 [T1] "sleep"이라는 이름을 쓰는 2026년 연구가 이것만은 아니다. 헷갈리기 쉬워
구분해 둔다.

| 연구 | 무엇을 자는 동안 하나 | 이 장과의 관계 |
|---|---|---|
| **Language Models Need Sleep** (이 절) | 빠른 모듈 → 느린 파라미터로 증류 + dreaming | CMS(`3.4`) 위에 오프라인 층을 얹는다 |
| **Sleep-Like Memory Consolidation** | attention cache가 차면 잠들어, 밀려날 문맥을 SSM 블록의 **fast weight**로 N번 반복해 압축한 뒤 KV를 비운다 | `02-linear-attention`의 고정 상태에 더 가깝다. **"재우는 횟수 N"이 곧 추론 능력**이 되고, wake 지연은 N과 무관하게 일정하다고 보고한다 |
| **Learning to Forget** 계열 | 무엇을 **버릴지**를 학습한다 | 스트리밍·장기 운용에서 낡은 문맥이 쌓이는 문제를 겨냥 |

> 💡 세 갈래가 공통으로 건드리는 전제는 하나다 — **"추론은 상태를 남기지 않는
> 순수 함수"라는 가정**. 그 가정이 깨지면 서빙은 "요청을 처리하는 시스템"에서
> **"모델을 계속 키우는 시스템"** 이 된다.
...
| | |
|---|---|
| **핵심 아이디어** | 갱신을 입력 처리 중에만 하지 않는다. **입력이 없는 시간대**에 빠른 모듈의 지식을 느린 파라미터로 증류하고(consolidation), 스스로 만든 데이터로 복습한다(dreaming) |
| **장점** | · in-context로 배운 것이 **문맥이 끝나도 남는다**<br>· 덮어쓰지 않고 **low-rank expert를 추가**하므로 옛 지식을 지킨다<br>· consolidation·dreaming이 **사용자 지연 경로 밖**에서 돈다<br>· 연속 학습·지식 주입·few-shot에서 SEAL·TTT·ICL 대비 개선 보고 |
| **한계** | · ⚠️ **연구 단계** — `3.2`~`3.4`와 같은 주의가 그대로 적용된다<br>· ⚠️ **sleep의 비용(시간·FLOPs·메모리)이 보고되지 않았다**<br>· 잠들 때마다 파라미터가 늘어난다 — 무한정 늘릴 수는 없다<br>· 💡(이 위키의 추정) 사용자·테넌트마다 가중치가 갈라져 서빙·롤백·검증 구조가 복잡해진다<br>· dreaming은 **자기 평가로 자기를 바꾼다** — 평가가 틀리면 그대로 굳는다 |
| **대표 모델** | 연구 단계 (Behrouz et al., Titans·ATLAS·HOPE와 같은 저자 그룹) |
| **다음으로** | 이 모든 것이 서빙 스택의 어떤 전제를 깨는지 → **시스템 관점** |

---

## 3.6 시스템 관점 — 추론 중에 학습이 일어난다

💡 **이 절부터 이 파일 끝까지는 대부분 이 위키의 시스템 분석이다.** Titans·ATLAS·HOPE·
Sleep 논문은 서빙 시스템에 대한 함의를 직접 다루지 않는다 (네 논문 모두 프로덕션
서빙 벤치마크를 보고하지 않는다). 아래 내용은 `3.1`~`3.5`에서 확인한 📌 사실(메모리
...
| **Sleep** | **online + offline write** | 위와 같되, 쉬는 시간에 자기 지식을 다시 읽음 | 입력이 없는 시간대에 빠른 기억을 **파라미터로 굳힌다** | 언제 재우고, 갈라진 가중치를 어떻게 관리할까 |
...
묻는다. `3.5`의 Sleep은 그 시간축을 서빙 바깥까지 늘여 **"언제 쓰기를 할 것인가"** 를
묻는다 — 이 프레이밍 자체는 논문의 것이 아니라 이 위키가 압축한 것이다.
...
| **오프라인 consolidation 슬롯** | `3.5`의 sleep은 서빙과 같은 자원을 쓰지만 사용자 요청은 처리하지 않음 | 유휴 시간대 예약, 우선순위 낮은 백그라운드 큐, 중단·재개 가능한 증류 |
...
- Behrouz, Hashemi, Javanmard, Mirrokni (2026), *Language Models Need Sleep: Learning to
  Self-Modify and Consolidate Memories*, arXiv:2606.03979 — wake/sleep 분리, CMS 위의
  오프라인 consolidation, knowledge seeding(파라미터 확장 + upward distillation),
  dreaming(합성 커리큘럼 + 격리 평가), AIME·SQuAD·ARC·CTNL·BABILong 결과
- Lee, McLeish, Goldstein, Fanti (2026), *Sleep-Like Memory Consolidation in LLMs*,
  arXiv:2605.26099 — KV cache가 차면 잠들어 SSM 블록의 fast weight로 압축, sleep 횟수 `N`과
  추론 능력의 관계 (`3.5` "같은 이름, 다른 갈래" 참고)
- *Learning to Forget: Sleep-Inspired Memory Consolidation* (2026), arXiv:2603.14517
  — 선택적 망각 쪽 갈래
...
- Titans·ATLAS·HOPE는 연구 제안이다. 대규모 프로덕션 서빙에서의 비용과 안정성은
  아직 확립된 사실로 다루지 않는다.
- `3.6`의 update engine과 다중 주기 메모리 계층은 논문의 구조에서 도출한
...
- `3.6`(시스템 관점) 이후는 이 파일에서 **가장 추정이 많은 구간**이다. Titans·ATLAS·
  HOPE·Sleep 논문 모두 서빙 인프라와의 통합이나 가속기 설계를 다루지 않는다 — `3.6`의
  다섯 가지 중 ①②③, "압력의 관점에서", "미래 accelerator" 표는 논문의 구조적 사실에서
  이 위키가 논리적으로 도출한 **추정**이고, 실제로 이렇게 배포되거나 측정된 적은 없다.
- `3.5`의 Sleep은 **비용 측면이 특히 비어 있다.** 논문은 성능 개선은 보고하지만
  한 번의 sleep에 드는 시간·FLOPs·메모리를 제시하지 않는다. "사용자 지연 경로 밖"이라는
  서술은 지연에 대한 것이지 **총비용이 공짜라는 뜻이 아니다.**
- `3.5`의 "시스템에서 새로 생기는 질문"(가중치가 갈라진다, 롤백, 재우는 시점)은
  전부 이 위키의 추정이다. 논문은 멀티테넌트 서빙 시나리오를 다루지 않는다.
