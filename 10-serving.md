...
| **희소 계열** (`01` 1.7~1.10) | 저장은 그대로지만 **읽기가 흩어진다** |
| **하이브리드** (`02` 2.6) | **층마다 다르다** — linear 층은 KV가 아예 없다 |
| **CLA / YOCO** (`01` 1.11) | 레이어 그룹이 페이지를 공유 |
...
| **레이어 간 KV 공유는 PP를 제약한다** | `01` 1.11 (CLA·YOCO) |
| **하이브리드는 층마다 자원 프로파일이 달라 균형이 안 맞는다** | `02` 2.6 |
