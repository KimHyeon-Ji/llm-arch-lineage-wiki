...
| **MLA** (`01` 2.3) | latent `c` 하나 + 공유 `k_R` — **구조가 다르다** |
| **SWA** (`01` 3.1) | 원형 버퍼. 오래된 페이지를 회수 |
| **희소 계열** (`01` 3.2~3.5) | 저장은 그대로지만 **읽기가 흩어진다** |
| **하이브리드** (`02` 3.1) | **층마다 다르다** — linear 층은 KV가 아예 없다 |
| **CLA / YOCO** (`01` 4.1) | 레이어 그룹이 페이지를 공유 |
...
왜 어려운지는 `02-linear-attention` `1.1`에서 본 구조 때문이다.
...
| 제약 | 출처 |
|---|---|
| **TP degree ≤ `n_kv`** — 아니면 KV를 복제해야 한다 | `01` 2.1~2.2 (MQA·GQA) |
| MLA는 latent에 헤드 차원이 없어 **TP 분할 방식이 다르다** ⚠️ | `01` 2.3 |
| **레이어 간 KV 공유는 PP를 제약한다** | `01` 4.1 (CLA·YOCO) |
| **하이브리드는 층마다 자원 프로파일이 달라 균형이 안 맞는다** | `02` 3.1 |
