...
BF16 70B 모델의 이론적 weight 크기는 약 140 GB다. 그 140 GB를 읽어오는 동안 실제로
하는 계산은 **바이트당 1 FLOP 남짓**인데, H100 SXM이 연산과 대역폭의 균형을 이루려면
**바이트당 약 295 FLOP**이 필요하다.

필요한 양의 `1/295`만 하고 있다는 뜻이다. 즉 이렇게 된다.
...
토큰 하나를 태우는 `(1, d) × (d, n)` GEMV나, 토큰 4개를 한꺼번에 태우는
`(4, d) × (d, n)` GEMM이나 **읽어야 할 가중치는 똑같다.** 계산량만 4배가 되는데
그 계산 능력은 어차피 놀고 있었으니, **걸리는 시간은 거의 그대로다.**
...
```
 ① draft:  작은 모델이 γ개 토큰을 빠르게 추측  (여기서는 γ=4)
            "the cat sat on"

 ② verify: 큰 모델이 그 4개 + 그 다음 자리까지 γ+1 = 5개 위치를
            **한 번의 forward로 동시에** 채점
            the ✓   cat ✓   sat ✓   on ✗

 ③ accept: 맞는 데까지 채택, 처음 틀린 자리는 큰 모델의 답으로 교체
...
거부 샘플링(rejection sampling)을 쓰기 때문이다. 규칙 자체는 한눈에 들어온다.

```
 draft가 토큰 x를 확률 q(x)로 뽑았고, target에서의 확률이 p(x)라면

   p(x) ≥ q(x)  →  그냥 채택
                   (target이 draft보다 더 좋아하는 토큰이니 받아도 손해가 없다)

   p(x) <  q(x)  →  p(x)/q(x) 확률로만 채택, 나머지는 거부
                   (draft가 과하게 밀어준 만큼만 깎아낸다)

 거부했을 때는 남은 차이 max(0, p − q)를 정규화한 분포에서 다시 뽑는다
```

**깎아낸 몫을 거부할 때 정확히 되돌려주기 때문에**, 최종적으로 나오는 토큰의
분포가 target 단독 샘플링과 같아진다. 속도를 위해 품질을 조금 내주는 거래가
아니라, **수학적으로 같은 것을 더 빨리 얻는 것**이다.
...
**이 반복이 끝난다는 보장은 의외로 간단한 데서 온다.** 한 스텝마다 **맨 앞의
미확정 위치는 반드시 제 값으로 확정된다** — 그 앞이 이미 전부 확정된 상태에서
계산한 값이니 순차 생성의 답과 같을 수밖에 없다. 그래서 최악이라도 순차 생성과
같은 횟수면 끝나고, 운이 좋으면 한 스텝에 여러 자리가 한꺼번에 맞아떨어진다.
**스텝 수가 순차 생성보다 많아질 일은 없고, 줄어드는 것만 운에 달린** 구조다.
...
대신 대가가 있다. `8.1`의 검증 위치 수가 `γ+1`이었다면 여기서는 **트리의 노드
개수**가 그 자리를 대신한다. 트리를 넓게 펼칠수록 검증 한 번에 태우는 위치가
그만큼 늘어난다는 뜻이다 — 놀던 연산 능력을 쓰는 것이라 어느 선까지는 거의
공짜지만, 그 선을 넘으면 그냥 낭비다. **"어느 가지를 키울 것인가"** 가 뒤에서
볼 EAGLE-2의 문제의식이 되는 이유다.
...
옆자리가 무엇으로 정해질지 모르는 채로, 자기 자리의 **주변분포**만 보고 고른다.

```
 다음 두 자리에 올 수 있는 말:  "New York"  또는  "San Francisco"

 자리 1의 주변분포:  New 0.5 / San 0.5
 자리 2의 주변분포:  York 0.5 / Francisco 0.5

 두 자리를 따로 뽑으면 →  "New Francisco" 가 25% 확률로 나온다
```

각 자리는 자기 분포에 충실했는데 합쳐놓으면 말이 안 된다. **결합분포를
주변분포의 곱으로 근사한 대가**이고, 한 번에 많이 확정할수록 심해진다.
...
한 분포에 뭉개져서, 어느 갈래에도 정확히 맞지 않는 토큰이 나온다. 앞에서 본
"New Francisco"가 정확히 이것이고, **블록 뒤쪽으로 갈수록 앞에 쌓인 갈래가
많아지니 더 심해진다.**
...
📌 [T1] backbone이 이미 만들어둔 hidden과 직전 토큰만 보고 **"이 자리, 통과할까?"를
0~1로 찍는 작은 이진 분류기**를 하나 붙인다. draft 위치 `k`마다 스칼라 하나가 나온다.
...
**곱셈이라는 것 자체가 직관이다.** `τ`는 step 하나에서 건지는 토큰 수,
`SPS(B)`는 초당 도는 step 수 — 곱하면 **초당 토큰 수**다. 검증을 길게 잡으면
`τ`는 오르지만 batch가 커져 `SPS(B)`가 떨어진다. **두 값의 곱이 꺾이는 지점**이
지금 이 부하에서의 최적 검증 길이다.
