# 07. Decoding — 한 스텝에 토큰 하나여야 하나

`00-foundations` `0.6`에서 본 사실을 다시 꺼내보자.

decode에서 토큰 하나를 만들려면 **모델 가중치를 전부 읽어야 한다.** 70B 모델이면
140 GB다. 그런데 그걸로 하는 계산은 바이트당 1 FLOP 남짓이고, H100의 균형점은 150쯤이다.
...
| | 절 | 한 줄 |
|---|---|---|
| [7.1](#71-speculative-decoding--추측하고-검증하기) | **Speculative Decoding** | 추측하고 검증하기 |
| [7.2](#72-medusa와-eagle--draft-모델-없이) | **Medusa · EAGLE** | draft 모델 없이 |
| [7.3](#73-mtp--학습-목표를-바꾸다) | **MTP** | 학습 목표를 바꾸다 |
| [7.4](#74-시스템-관점--acceptance-rate가-모든-것을-정한다) | **시스템 관점** | acceptance rate가 전부 ★ |

---

## 7.1 Speculative Decoding — 추측하고 검증하기
...
```
> **그림 7.1** — 3개를 채택하면 큰 모델의 forward 한 번으로 4개 토큰이 나온다.
...
> `01`의 KV 압축도, `04`의 MoE도, `06`의 parallel block도 전부 무언가를 내줬다.
...
| **한계** | · **acceptance rate에 성패가 달렸다** — 낮으면 오히려 손해<br>· draft 모델을 따로 두고 관리해야 한다<br>· 거부된 토큰의 KV 계산이 낭비<br>· **배치가 크면 이득이 줄어든다** (`7.4`) |
| **대표 구현** | vLLM, SGLang, TensorRT-LLM 등 주요 프레임워크 전반 |
| **다음으로** | draft 모델을 따로 두는 부담을 없애려면 → **Medusa · EAGLE** |

---

## 7.2 Medusa와 EAGLE — draft 모델 없이
...
DeepSeek-V3는 `k`=1 추가(즉 t+1과 t+2)를 썼고, Step 3.5 Flash는 MTP-3,
GLM·MiniMax 계열도 채택했다.
...
| **대표 모델** | **DeepSeek-V3 / V3.2 / V4** · GLM-4.7 · MiniMax M2.1 · **Step 3.5 Flash**(MTP-3) · Xiaomi MiMo-V2-Flash |
| **다음으로** | 이 모든 것의 효과는 하나의 숫자에 달려 있다 → **acceptance rate** |

---

## 7.4 시스템 관점 — acceptance rate가 모든 것을 정한다
...
> `04-moe` `4.4`의 granularity도, `06-shape` `6.1`의 depth도 같은 구조였다.
...
`08-numerics.md`는 다른 질문을 던진다. **값 하나에 몇 비트를 쓸 것인가.**
...
**T3 — 참고**
- Sebastian Raschka, 분기별 아키텍처 리뷰 — MTP 채택 현황(MTP-1 vs MTP-3)

**미검증 항목**
- Step 3.5 Flash의 MTP-3 세부 — 2차 자료 기반
- 작업 종류별 acceptance rate 수치 — 구체 값은 인용하지 않음
